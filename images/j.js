...
function randomRange(min,max) {
 ...
 Player.prototype.checkDirection  = function () {
    var newDrawX = this.drawX,
        ...
        if (!obstacleCollision && !outOfBounds(this, newDrawX, newDrawY)) {
          this.drawX = newDrawX;
          this.drawY = newDrawY; // allows the player to move to the position
 ...
 Player.prototype.checkObstacleCollide  = function (newDrawX, newDrawY) {
   ...
   for (var i = 0; i < obstacles.length; i++) {
    ...
    Player.prototype.checkShooting = function () {
        if (this.isSpacebar && !this.isShooting) {  /// to press spacebar or check if already shooting
 ...
 function checkKey(e, value) {
   ...
   if (keyID === 38) { // 38 is key code for up arrow
     player1.isUpKey = value;
     e.preventDefault();

   }
   if (keyID === 39) { // 39 is key code for Right arrow
     player1.isRightKey = value;
     e.preventDefault();

   }
   if (keyID === 40) { // 40 is key code for down arrow
     player1.isDownKey = value;
     e.preventDefault();

   }
   if (keyID === 37) { // 37 is key code for left  arrow
     player1.isLeftKey = value;
     e.preventDefault();

   }
   if (keyID = 32) { // 32 is key code for Spacebar.
...
function Enemy(){
   ...
   this.srcY = 600;
   this.width = 45;
   this.height = 54; // on the source image ie spritesheet image these values are for the enemy
...
function initEnemies() {
 ...
 Enemy.prototype.setTargetLocation = function (){
    this.randomMoveTime = randomRange(4000, 10000);
    var minX = this.centerX - 50, // check in all directions to move -50 pixels
        maxX = this.centerX + 50, // +50 pixels
 ...
 Enemy.prototype.die = function (){
    var soundEffects = new Audio("audio/dying.wav");
    soundEffects.play();
    clearInterval(this.moveInterval); // because the dead bodies will move if this function is not applied..
    this.srcX = 185; // dead enemy sprite location @ 185 on spritesheet..
    this.isDead = true;
